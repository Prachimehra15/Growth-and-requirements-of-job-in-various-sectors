...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    if(empty($input_level)){
        $level_err = "Please enter the level ";     
    } else{
        $level = $input_level;
    }
    
    // Check input errors before inserting in database
    if(empty($name_err) && empty($email_err) && empty($level_err)){
        // Prepare an update statement
        $sql = "UPDATE users SET username=?, email=?, level=? WHERE id=?";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssi", $param_name, $param_email, $param_level, $param_id);
